...
1. 并$\vee$(要满足并相容性)  
   数学描述：
   $$
   R\vee S= \left\{ t|t\in R \vee t \in S \right\}
   $$
2. 差$-$(要满足并相容性)  
   数学描述：
   $$
...
3. 广义笛卡尔积操作  
   数学描述：
   $$
   关系R< a_1, a_2, \cdots , a_n>, 关系S<b_1, b_2, \cdots , b_n>, 则 \\ R \times S = \left\{ <a_1, a_2, \cdots , a_n, b_1, b_2, \cdots , b_n> | <a_1, a_2, \cdots , a_n> \in R \land <b_1, b_2, \cdots , b_n> \in S \right\}
   $$
4. 选择操作  
   数学描述：
   $$
   \delta _{con}(R)=\left\{ t | t \in R \land con(t) = true \right\}
   $$
5. 投影  
   数学描述：
   $$
   \Pi_{a_{i1}, a_{i2}, \cdots, a_{ik}}(R) = \left\{ <t[A_{i1}],t[A_{i2}], \cdots , t[A_{ik}]> | t \in R \right\}
   $$
   **示例**  
...
1. 交
   数学描述：
   $$R \cap S= \left\{ t|t\in R \land t \in S \right\}$$
2. $\theta$-连接
   第一步：对两个表进行广义笛卡尔积  
...
1. 除操作$\div$
   **前提条件**  
   若$R(A_1, A_2, \cdots,A_n)$为n度关系，关系$S(B_1, B_2, \cdots, B_n)$为m度关系，只有当$\{B_1, B_2, \cdots, B_n\} \subseteq \{A_1, A_2, \cdots,A_n\} $即B是A的真子集，$m<n$时才可进行$R\div S$运算  
   **定义**   
