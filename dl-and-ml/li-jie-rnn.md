...
与Keras中所有的循环层一样，SimpleRNN可以在两种不同的模式下运行：一种是返回每个时间步连续输出的完整序列，即形状为（batch\_size, timesteps, output\_features）的三维张量；另一种是只返回每个输入系列的最终输出，即形状为\(batch\_size, output\_featrues\)的二维张量。这个两种模式由return\_sequences这个构造函数参数来控制。
